...
export const LogoMarquee = () => {
  return (
    <section className="relative py-24 bg-white overflow-hidden border-t border-b border-sadhana-sand">
        ...
        <div className="flex w-1/2 md:w-max justify-around animate-[marquee_20s_linear_infinite] group-hover:[animation-play-state:paused]">
          {PARTNERS.map((partner, idx) => (
            <div key={`marquee-1-${idx}`} className="flex items-center justify-center px-12">
              <span className="text-2xl md:text-4xl font-black tracking-widest text-sadhana-dark opacity-30 hover:opacity-100 transition-opacity whitespace-nowrap">
                {partner}
              </span>
              {/* Separator Dot */}
