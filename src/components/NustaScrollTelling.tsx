...
export const NustaScrollTelling: React.FC = () => {
  ...
  return (
    <>
      ...
      <section ref={sectionRef} id="leyenda" className="relative py-20 bg-[#0a0a0a] text-stone-200 border-b border-stone-800 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-natgeo-yellow/30 bg-natgeo-yellow/10 text-natgeo-yellow text-xs uppercase tracking-widest mb-3 font-mono font-bold shadow-sm">
            <Video className="w-3.5 h-3.5 text-natgeo-yellow" />
          ...
          <h2 className="font-cinzel text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-white">
            La Leyenda de la <span className="natgeo-text">Ñusta Encantada</span>
          </h2>
          <p className="mt-4 text-stone-400 text-sm sm:text-base leading-relaxed font-sans font-medium">
          ...
          {STORY_ACTS.map((act, idx) => (
            <button
              ...
              className={`p-3.5 rounded-2xl border text-left transition-all duration-300 flex flex-col justify-between cursor-pointer ${
                activeActIndex === idx
                  ? 'bg-stone-900 border-natgeo-yellow shadow-md ring-1 ring-natgeo-yellow/50'
                  : 'bg-black/50 border-stone-800 hover:border-stone-600'
              }`}
            >
              <div>
                <div className="flex items-center justify-between text-xs text-stone-500 mb-1">
                  <span className={`font-mono font-bold text-[10px] ${activeActIndex === idx ? 'text-natgeo-yellow' : 'text-stone-500'}`}>ACTO 0{act.id}</span>
                  <span className="text-[10px] text-stone-500 font-mono font-medium">{act.stoneProgress}% Roca</span>
                </div>
                <h3 className={`font-cinzel text-xs sm:text-sm font-bold line-clamp-1 ${activeActIndex === idx ? 'text-white' : 'text-stone-400'}`}>
                  {act.title}
                </h3>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-black/60 border border-stone-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl">
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <p className="text-stone-300 bg-stone-900/50 border-l-4 border-natgeo-yellow p-3 rounded-r-xl text-xs sm:text-sm font-serif italic">
              {activeAct.quote}
            </p>
            ...
            <motion.p 
              ...
              className="text-stone-400 text-xs sm:text-sm leading-relaxed font-sans font-mono typewriter-cursor"
            >
